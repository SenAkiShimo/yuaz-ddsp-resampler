...
from yuaz_ddsp_resampler.core import YuazDDSPResamplerEngine, make_adaptive_decoder_class, decode_int12_pitch, tone_to_midi, crop_oto, build_target_f0, extract_detail_features, piecewise_warp, blend_dualrate_fullband_body, blend_dualrate_fullband_body_v2
...
assert parse_yuaz_controls("YA100") == parse_yuaz_controls(""), "YA Attack must be fully retired in 0.2.8ai.12"
_manifest = yaml.safe_load(Path("resampler-manifest.yaml").read_text(encoding="utf-8"))
_exprs = _manifest.get("expressions", {})
assert len(_exprs) == 12, f"0.2.8ai.12 must expose exactly 12 Yuaz controls, got {len(_exprs)}"
...
# 0.2.8ai.12 keeps a low-gain interpretable carrier underneath learned packs so
...
assert "voicebank_registry-0.2.8ai12.json" in _configure_txt
assert "voicebank_registry-0.2.8ai6.json" not in _configure_txt
assert 'voicebank_registry-0.2.8ai12.json' in _purge_txt
assert ".yuaz-0.2.8ai12" in _purge_txt and ".yuaz-0.2.8ai11" in _purge_txt and ".yuaz-0.2.8ai9" in _purge_txt and ".yuaz-0.2.8ai8" in _purge_txt and ".yuaz-0.2.8ai7" in _purge_txt and ".yuaz-0.2.8ai6" in _purge_txt
...
# Previous 0.2.8ai.11 is the immediate read-only predecessor and must win over older state.
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev11 / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai.11 fallback state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev10 / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai.10 fallback state"
...
# Migration primitive regression: model/training state is copied from ai.11,
# while derived caches are deliberately omitted and regenerated by ai.12.
with tempfile.TemporaryDirectory() as td:
    bank = Path(td) / "migration-bank"; bank.mkdir()
    src = bank / PREVIOUS_028AI11_STATE_CONTAINER / "generations" / "oldgen"
    make_minimal(src)
    ...
    generation, staging = begin_generation(bank, "migrate11")
    clone_state(src, staging, link_caches=False, skip_caches=True)
    final, _ = commit_generation(bank, generation, staging, reason="migrated-from-0.2.8ai.11", acoustic_base="0.2.8ai.12-upperband-parameter-head-v1")
    ...
    assert not (final / "cache").exists(), "ai.12 migration must rebuild derived analysis caches"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev5 / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai.5 fallback state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev4 / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai.4 fallback state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev3 / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai.3 fallback state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev2 / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai.2 fallback state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev1 / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai.1 fallback state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (prev / "runtime_registry.json").exists(), "0.2.8ai.12 must not write into 0.2.8ai fallback state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    assert not (pred / "runtime_registry.json").exists(), "0.2.8ai.12 must not write cache files into AI.3 fallback state"
    assert hashlib.sha256((pred / "profile.json").read_bytes()).hexdigest() == before

# Predecessor/stable fallback isolation: 0.2.8ai.12 may READ 0.2.8ai.4/0.2.8ai.3/0.2.8ai.2/0.2.8ai.1/0.2.8ai/AI.3/RC4.2 state when its own state is absent,
# but committing a generation must create only .yuaz-0.2.8ai12.
with tempfile.TemporaryDirectory() as td:
...
# 0.2.8ai.12 dual-rate DDSP regression. The frozen 24 kHz neural envelope is
...
# Exact-source preservation: the complete ai.11 package snapshot ships inside
# ai.12, satisfying the no-delete versioning policy requested for this release.
_ai11_snapshot=Path('previous_versions/v0.2.8ai.11')
assert (_ai11_snapshot/'VERSION').read_text(encoding='utf-8').strip()=='0.2.8ai.11'
assert (_ai11_snapshot/'src/yuaz_ddsp_resampler/core.py').is_file()
assert (_ai11_snapshot/'scripts/install-openutau-macos.command').is_file()

print("0.2.8ai.12 upper-band parameter head + band-wise 48 kHz mixer + Foundation refinement self-test OK")
PY
