...
mkdir -p "$APP_STATE"
python3 - "$YUAZ" "$CKPT" "$APP_STATE/voicebank_registry-0.2.8ai12.json" <<'PY'
import json, sys
...
config = {
    ...
    "runtime_id": "yuaz-0.2.8ai.12-control-v12",
    ...
    "state_namespace": ".yuaz-0.2.8ai12",
...
print("Migration source order before purge: 0.2.8ai.11 / 0.2.8ai.10 / 0.2.8ai.9 / 0.2.8ai.8 / 0.2.8ai.7 / 0.2.8ai.6 / 0.2.8ai.5 / 0.2.8ai.4 / 0.2.8ai.3 / 0.2.8ai.2 / 0.2.8ai.1 / 0.2.8ai / AI.3 / RC4.2; new state writes only .yuaz-0.2.8ai12")
print("Dual-rate DDSP: 24 kHz analysis + 48 kHz synthesis body + frequency-dependent upper-band parameter head; YH is refinement")
