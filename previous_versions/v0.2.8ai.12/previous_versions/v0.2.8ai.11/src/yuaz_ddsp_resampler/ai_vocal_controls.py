...
class AIControlAdapter(nn.Module):
    ...
    def apply(self, spectral_envelope, ap_bands, gate, f0, controls):
        ...
        # A checkpoint can minimize reconstruction loss by regressing toward a very
        # small average residual. 0.2.8ai.11 applies a bounded runtime calibration so
        # a non-zero learned control cannot become perceptually indistinguishable.
        with torch.no_grad():
...
def load_ai_control_adapter(path, device="cpu", expected_controls=None):
    ...
    # Existing 0.2.8ai.5 checkpoints do not carry perceptual calibration metadata.
    # Use conservative pack-specific defaults; explicit future metadata overrides it.
    defaults = {
