...
def synthesize_learned_highband(generated, sr, target_f0, profile, seed, assist_start_hz=10000.0, detail_strength=1.0, target_fixed_ms=0.0, restoration_strength=1.0):
    ...
    # The Yuaz/DDSP body runs at 24 kHz, so its physical Nyquist limit is 12 kHz.
    # A source-trained profile can therefore be near-zero above 12 kHz even when
    # the original WAV had useful edge energy. v0.2.8ai.11 anchors the extension
    # to the rendered 7–11 kHz edge and applies a conservative reconstruction
    # floor instead of blindly reproducing a missing >12 kHz profile.
    ...
    # v0.2.8ai.11: do not draw the upper band as a bank of mathematically perfect
    # sinusoids. That created the ruler-straight harmonic ladder visible in a
    # spectrogram and it also penalized low notes because high-frequency energy
    # was divided by harmonic index and capped by a fixed partial count.
    # Instead, create a band-limited nonlinear excitation from the rendered
    # voice itself. Quadratic/cubic paths generate even+odd upper partials with
    # the source waveform's real phase, articulation and formant motion.
    ...
    # Keep each nonlinear path below the frequency that would fold back from
    # Nyquist. The square path can safely start from ~9.5 kHz; the cubic path
    # is intentionally limited to ~6.3 kHz.
    ...
    # Real recordings do not contain infinitely thin, perfectly stable upper
    # partials. Add deterministic narrow spectral skirts around the excitation
    # peaks before applying the learned voicebank envelope. This keeps pitch
    # coherence but removes the synthetic ruler-line appearance.
    ...
    # Diagnostic estimate only: the new texture path has no fixed partial-count
    # ceiling, so low notes can naturally populate the whole 12–20 kHz band.
