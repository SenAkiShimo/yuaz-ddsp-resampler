...
ENGINE_VERSION = "0.2.8ai.11"
STATE_CONTAINER = ".yuaz-0.2.8ai11"
...
def resolve_ai_state(bank, verify=True):
    return _resolve_generation_container(bank, STATE_CONTAINER, verify=verify, source_label="0.2.8ai.11")
...
def resolve_active_state(bank, allow_legacy=True, verify=True):
    # 0.2.8ai.11 owns only its own namespace. If it has no active generation yet,
    # it may read 0.2.8ai.10 and older prepared states strictly read-only.
    # This lets the new engine render the user's already-Deep-trained voicebank
    # immediately without mutating or re-signing the predecessor state.
...
def rollback_to_previous(bank):
    ...
    if not previous:
        raise RuntimeError("No previous 0.2.8ai.11 generation is available to roll back to.")
