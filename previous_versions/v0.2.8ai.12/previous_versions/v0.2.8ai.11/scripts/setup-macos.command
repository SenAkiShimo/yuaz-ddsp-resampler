...
ENVROOT="$APP/environments/0.2.8ai.11-$HASH"
PREV_ENV="$APP/environments/0.2.8ai.10-$HASH"
...
if [ ! -x "$ENVROOT/bin/python" ] && [ -x "$PREV_ENV/bin/python" ] && verify_env "$PREV_ENV"; then
  ENVROOT="$PREV_ENV"
