...
def create_backup(project_root, voicebank, reason="manual"):
    ...
    dest = base / f"{stamp}-before-0.2.8ai.12-{_safe_name(reason)}"
    suffix = 1
    while dest.exists():
        dest = base / f"{stamp}-before-0.2.8ai.12-{_safe_name(reason)}-{suffix}"
        suffix += 1
    ...
    archive_state("0.2.8ai.12", ai_active, ai_info, "0.2.8ai.12-runtime-state.tar.gz", STATE_CONTAINER, include_caches=False)
    ...
    manifest = {
        ...
        "cache_policy": "Current 0.2.8ai.12 routine cache is excluded; predecessor 0.2.8ai.7, 0.2.8ai.5, 0.2.8ai.4, 0.2.8ai.3, 0.2.8ai.2, 0.2.8ai.1, 0.2.8ai, AI.3 and RC4.2 containers are archived separately in full, including retained generations and caches",
...
def restore_backup(backup_dir, voicebank, target="ai"):
    ...
    if target != "ai":
        raise RuntimeError("0.2.8ai.12 restore only supports target=ai or rc3-2; stable RC4.2 is deliberately not modified by this branch.")
    archive = backup_dir / "0.2.8ai.12-runtime-state.tar.gz"
    if not archive.exists():
        raise RuntimeError("Backup has no 0.2.8ai.12 state. Stable RC4.2 snapshots are read-only safety copies in this branch.")
    ...
    try:
        ...
        src = temp / "0.2.8ai.12-state"
        if not src.is_dir():
            raise RuntimeError("0.2.8ai.12 backup archive is malformed.")
        for child in src.iterdir():
            shutil.move(str(child), str(staging / child.name))
        shutil.rmtree(temp)
        (staging / "runtime_registry.json").unlink(missing_ok=True)
        final, _ = commit_generation(bank, generation, staging, reason="external-ai-backup-restore", acoustic_base="0.2.8ai.12-twelve-control-modular-ai-ddsp")
        return final
    except Exception:
