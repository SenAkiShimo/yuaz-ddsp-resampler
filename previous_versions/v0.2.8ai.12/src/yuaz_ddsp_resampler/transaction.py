    ...
    if status.get("ready"):
        ...
        while time.time()<deadline:
            if not ping(host,port):
                return
            time.sleep(0.1)
        raise RuntimeError(f"0.2.8ai.12 engine would not stop cleanly during {phase}; ACTIVE was not changed.")
...
def _attach_ai_control_foundation(project_root, staging, source_state=None):
    ...
    if feature_backend != "yuaz-native-ddsp-v1":
        ...
        print("AI Control Foundation rejected: 0.2.8ai.12 requires Yuaz-native DDSP training features.")
        return None
    ...
    atomic_write_json(meta_path, {
        ...
        "migration_policy": "0.2.8ai.12 may reuse compatible frozen predecessor foundations without modifying their source",
        "frozen_during_voicebank_deep": True,
        "created_at": time.time(),
    })
    print(f"AI Control Foundation pinned into 0.2.8ai.12 generation from: {foundation}")
...
def _find_ai_gender_foundation(project_root, source_state=None):
    ...
    candidates.extend([
        ...
        home / "Library" / "Application Support" / "YuazDDSP" / "0.2.8ai.12" / "control_models" / "ai_gender_foundation-v1.pt",
        home / "Downloads" / "yuaz-ddsp-resampler-v0.2.8ai.12" / "control_models" / "ai_gender_foundation-v1.pt",
...
def _attach_ai_gender_foundation(project_root, staging, source_state=None):
    ...
    print(f"AI Gender Foundation pinned into 0.2.8ai.12 generation from: {foundation}")
...
def _find_modular_foundation(project_root, filename, backup_names=(), source_state=None, state_filename=None):
    ...
    for ver in ("0.2.8ai.12", "0.2.8ai.11", "0.2.8ai.3", "0.2.8ai.2", "0.2.8ai.1", "0.2.8ai"):
...
def _attach_modular_foundation(project_root, staging, *, filename, target_filename, metadata_filename,
    ...
    print(f"{label} pinned into 0.2.8ai.12 generation from: {foundation}")
    return target
...
def _attach_highband_foundation(project_root, staging, source_state=None):
    ...
    print(f"High-Band Foundation r{revision} pinned into 0.2.8ai.12 generation from: {foundation}")
...
def run_transaction(project_root, voicebank, mode):
    ...
    backup_dir, _ = create_backup(project_root, bank, reason=f"0.2.8ai.12-{mode}")
    ...
    try:
        if mode == "adopt":
            # "Adopt RC3.2 Baseline" is deliberately deterministic: if the legacy
            # RC3.2 state exists, always adopt that exact state even when RC3.3 has
            # already been tested. This gives the user a stable known baseline.
            ...
            reason = "0.2.8ai.12-production-clean-deep-plus-modular-control-packs"
        elif mode == "continue":
            if source is None:
            ...
            _attach_ai_mouth_foundation(project_root, staging, source_state=source)
            _attach_highband_foundation(project_root, staging, source_state=source)
            reason = "continue-0.2.8ai.12-deep-isolated-copy-plus-modular-control-packs"
        elif mode == "highband":
            if source is None:
        ...
        final, active = commit_generation(
            bank, generation, staging, reason=reason, acoustic_base="0.2.8ai.12-twelve-control-modular-ai-ddsp",
        )
