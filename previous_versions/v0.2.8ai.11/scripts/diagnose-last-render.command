...
INST="$HOME/Library/Application Support/YuazDDSP/0.2.8ai.11/logs/render_requests.jsonl"
LOCAL="$ROOT/logs/render_requests.jsonl"
LOG=""
[ -f "$INST" ] && LOG="$INST"
[ -z "$LOG" ] && [ -f "$LOCAL" ] && LOG="$LOCAL"
if [ -z "$LOG" ]; then
  echo "No 0.2.8ai.11 render log yet. Render at least one note in OpenUtau first."
  exit 0
fi
python3 - "$LOG" <<'PY'
import json,sys
...
print('Yuaz 0.2.8ai.11 recent render routing:',p)
for line in lines:
