...
DEST="$BASE/$STAMP-before-0.2.8ai.11"
mkdir -p "$DEST"
...

{
  ...
  echo "policy=pre-purge snapshots; 0.2.8ai.10, 0.2.8ai.9, 0.2.8ai.8, 0.2.8ai.7, 0.2.8ai.6, 0.2.8ai.5, 0.2.8ai.4, 0.2.8ai.3, 0.2.8ai.2, 0.2.8ai.1, 0.2.8ai, 0.2.7 AI.3 and RC4.2 are not stopped, removed, overwritten, or modified"
} > "$DEST/manifest.txt"
...
# Snapshot every current ai.10 voicebank state before the installer is allowed to purge it.
...
seen=set()
for state in root.rglob('.yuaz-0.2.8ai10'):
    if not state.is_dir():
        continue
    ...
    archive=out/f'{safe}-0.2.8ai10-state.tar.gz'
    def filt(info):
        rel=Path(info.name)
    ...
    with tarfile.open(archive,'w:gz') as tf:
        tf.add(state, arcname='.yuaz-0.2.8ai10', filter=filt)
    with (out/'manifest.txt').open('a',encoding='utf-8') as f:
        f.write(f'{bank}\t{archive.name}\n')
print(f'Voicebank ai.10 safety snapshots: {len(seen)}')
PY_STATE
fi
...
echo "$DEST" > "$BASE/LATEST_PRE_0.2.8AI11_ENGINE_SNAPSHOT.txt"
echo "Pre-0.2.8ai.11 engine snapshot created: $DEST"
echo "Snapshot includes current 0.2.8ai.10 and available predecessors before migration/purge. Previous installed versions may be removed after successful state migration. Included when present: 0.2.8ai.10 + 0.2.8ai.9 + 0.2.8ai.8 + 0.2.8ai.7 + 0.2.8ai.6 + 0.2.8ai.5 + 0.2.8ai.4 + 0.2.8ai.3 + 0.2.8ai.2 + 0.2.8ai.1 + 0.2.8ai + 0.2.7 AI.3 + RC4.2 runtimes/wrappers + existing voicebank state."
