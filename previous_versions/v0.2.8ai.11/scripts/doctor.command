...
assert c.get('engine_version')=='0.2.8ai.11'
assert int(c.get('port'))==47885
assert c.get('runtime_id')=='yuaz-0.2.8ai.11-control-v11'
assert c.get('state_namespace')=='.yuaz-0.2.8ai11'
...
assert float(c.get('ddsp_fullband_crossover_start_hz'))==9000.0
...
WRAP="$HOME/Library/OpenUtau/Resamplers/Yuaz-DDSP-Resampler-v0.2.8ai.11.sh"
...
if [ -d "$HOME/Library/OpenUtau/Resamplers" ]; then
  OLD="$(find "$HOME/Library/OpenUtau/Resamplers" -maxdepth 1 -type f \( -name 'Yuaz-DDSP-Resampler*.sh' -o -name 'Yuaz-DDSP-Resampler*.yaml' \) ! -name 'Yuaz-DDSP-Resampler-v0.2.8ai.11.sh' ! -name 'Yuaz-DDSP-Resampler-v0.2.8ai.11.yaml' -print)"
  if [ -n "$OLD" ]; then
...
echo "PASS: dual-rate body configured: 24 kHz analysis/latent + 48 kHz DDSP synthesis; YH is a refinement layer above the full-band body."
echo "Doctor OK"
