...
from .state import (
    ACTIVE_FILE, LEGACY_STATE, STATE_CONTAINER, PREVIOUS_028AI7_STATE_CONTAINER, PREVIOUS_028AI5_STATE_CONTAINER, PREVIOUS_028AI4_STATE_CONTAINER, PREVIOUS_028AI3_STATE_CONTAINER, PREVIOUS_028AI2_STATE_CONTAINER, PREVIOUS_028AI1_STATE_CONTAINER, PREVIOUS_028_STATE_CONTAINER, PREDECESSOR_AI_STATE_CONTAINER, STABLE_STATE_CONTAINER, atomic_write_json,
    begin_generation, commit_generation, resolve_active_state, resolve_ai_state, resolve_previous_028ai7_state, resolve_previous_028ai5_state, resolve_previous_028ai4_state, resolve_previous_028ai3_state, resolve_previous_028ai2_state, resolve_previous_028ai1_state, resolve_previous_028_state, resolve_predecessor_ai_state, resolve_stable_state,
)
...
def create_backup(project_root, voicebank, reason="manual"):
    ...
    dest = base / f"{stamp}-before-0.2.8ai.11-{_safe_name(reason)}"
    suffix = 1
    while dest.exists():
        dest = base / f"{stamp}-before-0.2.8ai.11-{_safe_name(reason)}-{suffix}"
        suffix += 1
    ...
    archive_state("0.2.8ai.11", ai_active, ai_info, "0.2.8ai.11-runtime-state.tar.gz", STATE_CONTAINER, include_caches=False)
    ...
    if stable_container.is_dir():
        # Full immutable safety copy: ACTIVE.json + every retained generation +
        # analysis/high-band caches. This is intentionally larger than routine AI
        # backups because RC4.2 is the user's known-good escape hatch.
    ...
    manifest = {
        ...
        "cache_policy": "Current 0.2.8ai.11 routine cache is excluded; predecessor 0.2.8ai.7, 0.2.8ai.5, 0.2.8ai.4, 0.2.8ai.3, 0.2.8ai.2, 0.2.8ai.1, 0.2.8ai, AI.3 and RC4.2 containers are archived separately in full, including retained generations and caches",
...
def restore_backup(backup_dir, voicebank, target="ai"):
    ...
    if target != "ai":
        raise RuntimeError("0.2.8ai.11 restore only supports target=ai or rc3-2; stable RC4.2 is deliberately not modified by this branch.")
    archive = backup_dir / "0.2.8ai.11-runtime-state.tar.gz"
    if not archive.exists():
        raise RuntimeError("Backup has no 0.2.8ai.11 state. Stable RC4.2 snapshots are read-only safety copies in this branch.")
    ...
    try:
        ...
        src = temp / "0.2.8ai.11-state"
        if not src.is_dir():
            raise RuntimeError("0.2.8ai.11 backup archive is malformed.")
        for child in src.iterdir():
            shutil.move(str(child), str(staging / child.name))
        shutil.rmtree(temp)
        (staging / "runtime_registry.json").unlink(missing_ok=True)
        final, _ = commit_generation(bank, generation, staging, reason="external-ai-backup-restore", acoustic_base="0.2.8ai.11-twelve-control-modular-ai-ddsp")
        return final
    except Exception:
