...
class VoicebankAdapter(nn.Module):
    ...
    def _pitch_weights(self, f0, batch=1, device=None, dtype=None, source_prototype_index=None):
        ...
        sigma = torch.median(spacing) * 0.70 if spacing.numel() else torch.tensor(4.0, device=device, dtype=dtype)
        sigma = torch.clamp(sigma, min=1.8, max=6.0)
        ...
        if source_prototype_index is not None:
            ...
            boost = 1.35 * proximity * valid.to(dtype)
            logits = logits.scatter_add(1, safe_idx.reshape(-1, 1), boost.reshape(-1, 1))
        return torch.softmax(logits, dim=1)

    def timbre_representation(self, f0=None, batch=1, source_prototype_index=None):
        device = self.timbre_code.device
        dtype = self.timbre_code.dtype
        weights = self._pitch_weights(f0, batch=batch, device=device, dtype=dtype, source_prototype_index=source_prototype_index)
        local = weights @ self.pitch_timbre_codes
        return self.timbre_code.view(1, -1) + 0.70 * local

    def apply_latent(self, z, detail=None, f0=None, source_prototype_index=None):
        ...
        out = out * scale + bias
        timbre = self.timbre_representation(f0=f0, batch=z.shape[0], source_prototype_index=source_prototype_index)
        out = out + 0.10 * torch.tanh(self.timbre_to_latent(timbre)).unsqueeze(-1)
        h = self._detail_hidden(detail, z.shape[-1])
        if h is not None:
            out = out + 0.08 * torch.tanh(self.detail_to_latent(h))
        return out

    def spectral_gain(self, bins, device=None, dtype=None, detail=None, frames=None, f0=None, batch=1, source_prototype_index=None):
        global_log_gain = torch.tanh(self.spectral_log_gain_raw) * 0.40
        timbre = self.timbre_representation(f0=f0, batch=batch, source_prototype_index=source_prototype_index)
    ...
    def apply_ap(self, ap, detail=None, f0=None, source_prototype_index=None):
        timbre = self.timbre_representation(f0=f0, batch=ap.shape[0], source_prototype_index=source_prototype_index)
    ...
    def apply_gate(self, gate, f0=None, source_prototype_index=None):
        timbre = self.timbre_representation(f0=f0, batch=gate.shape[0], source_prototype_index=source_prototype_index)
