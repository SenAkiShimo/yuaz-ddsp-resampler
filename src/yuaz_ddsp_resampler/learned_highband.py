...
def build_profile_database(voicebank_root, manifest_entries, model_hop=256, model_sr=24000, state_dir=None):
    ...
    return {
        ...
        "continuity_mode": "edge_anchored_slope_limited_v1",
...
def select_learned_profile(db, base_alias, target_midi, timbre_shift_semitones=0.0, source_prototype_index=None):
    ...
    if not db or not base_alias:

        return None

    group = (db.get("groups") or {}).get(str(base_alias))
...
def synthesize_learned_highband(generated, sr, target_f0, profile, seed, assist_start_hz=10000.0, detail_strength=1.0, target_fixed_ms=0.0):
    ...
    phase = np.cumsum(2.0 * np.pi * f0_safe / float(sr), dtype=np.float64)

    min_f0 = max(55.0, float(np.percentile(f0_safe[f0 > 1.0], 10)) if np.any(f0 > 1.0) else 110.0)

    bridge_start = max(11200.0, min(12000.0, float(assist_start_hz) + 1200.0))

    bridge_full = 13000.0

    k0 = max(2, int(math.floor(bridge_start / min_f0)) - 2)

    k1 = min(320, int(math.ceil(min(20000.0, sr * 0.5 - 300.0) / min_f0)) + 3)

    harmonic = np.zeros(y.size, dtype=np.float64)
    ...
    for k in range(k0, k1 + 1):
        ...
        active = (freq >= bridge_start) & (freq <= min(20000.0, sr * 0.5 - 300.0)) & (f0 > 1.0)

        if not np.any(active):

            continue

        shape = np.interp(freq, centers, voiced_amp, left=voiced_amp[0], right=voiced_amp[-1])

        cross = np.clip((freq - bridge_start) / max(200.0, bridge_full - bridge_start), 0.0, 1.0)

        cross = cross * cross * (3.0 - 2.0 * cross)

        harmonic += np.sin(k * phase) * shape * active * cross / math.sqrt(float(k))

        harmonic_count += 1

    if harmonic_count:

        harmonic_rms = float(np.sqrt(np.mean((harmonic * voiced) ** 2) + 1e-12))

        target_harmonic = float(np.sqrt(np.mean((y * voiced) ** 2) + 1e-12)) * float(np.mean(voiced_amp[2:])) * harmonic_mix

        if harmonic_rms > 1e-9:

            harmonic *= min(8.0, target_harmonic / harmonic_rms)
    ...
    noise_low_cut = max(9000.0, min(11800.0, float(assist_start_hz)))
    ...
    voiced_air = 0.05 + 0.32 * (1.0 - harmonic_mix_t)
    ...
    target_noise = max(
        ...
        voiced_body * float(np.mean(voiced_amp[2:])) * (1.0 - harmonic_mix) * 0.42,
    ...
    branch = (harmonic + shaped) * detail_gain

    branch_rms = float(np.sqrt(np.mean(branch ** 2) + 1e-12))

    safety = 1.0

    if body_rms > 1e-8 and branch_rms > 0.32 * body_rms:

        safety = 0.32 * body_rms / max(branch_rms, 1e-12)
