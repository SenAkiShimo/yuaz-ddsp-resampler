...
def make_adaptive_decoder_class(base_cls):
    class AdaptiveDDSPDecoder(base_cls):
        ...
        def _harmonic_oscillator_at_rate_output_guard(self, f0, n_samples, sample_rate, output_sample_rate, max_harmonics=512):
            """Band-limited oscillator with an output-rate harmonic ceiling."""
            B, _, _ = f0.shape
            device = f0.device
            sr = float(sample_rate)
            f0_up = F.interpolate(f0, size=int(n_samples), mode="linear", align_corners=False)
            ...
            if voiced_values.numel():
                ...
                min_f0 = 220.0
            tf = terminal_frequencies(int(output_sample_rate))
            ceiling = min(0.5 * sr - 20.0, float(tf["harmonic_ceiling_hz"]))
            needed = int(math.ceil(ceiling / max(20.0, min_f0)))
            count = max(int(self.n_harmonics), min(int(max_harmonics), max(1, needed)))
            out = torch.zeros_like(safe_f0)
            phase4 = phase.unsqueeze(2)
            f04 = safe_f0.unsqueeze(2)
            ...
            for k0 in range(1, count + 1, chunk):
                ...
                k = torch.arange(k0, k1, device=device, dtype=f0.dtype).view(1, 1, -1, 1)
                hz = f04 * k
                valid = ((hz < ceiling) & (hz > 0.0)).to(f0.dtype)
                out = out + ((torch.sin(k * phase4) / k) * valid).sum(dim=2)
            return out * voiced, count, float(ceiling)
        ...
        def _synthesize_fullband_body_upperband_head_v2(self, f0, S_lin, A_band, A_raw_band, gate, synthesis_sample_rate):
            ...
            ratio = float(sr) / float(self.sample_rate)
            if ratio <= 1.01:
                raise ValueError("ai.13 upper-band guard requires synthesis above the 24 kHz analysis body")
            output_sr = int(getattr(self, "ai13_output_sample_rate", 44100))
            synth_fft = int(round(self.fft_size * ratio))
            if synth_fft % 2:
                synth_fft += 1
            synth_hop = max(1, int(round(self.hop_length * ratio)))
            n_samples = max(1, int(round(f0.shape[-1] * self.encoder_hop_length * ratio)))
            e_h, harmonic_count, harmonic_ceiling = self._harmonic_oscillator_at_rate_output_guard(
                f0, n_samples, sr, output_sr
            )
            window = torch.hann_window(synth_fft, device=f0.device, dtype=f0.dtype)
            spec_h = torch.stft(e_h.squeeze(1), synth_fft, synth_hop, window=window, return_complex=True)
            spec_h = spec_h[:, :synth_fft // 2 + 1, :]
            ...
            S_full, spectral_guard_stats = apply_upperband_spectral_guard(S_full, sr, output_sr)
            A_full, ap_head_stats = extend_aperiodicity_upperband(
                A_low, gate, synth_fft, self.sample_rate, sr, high_target=0.82,
            )
            A_raw_full, ap_raw_head_stats = extend_aperiodicity_upperband(
            ...
            head_start = float(getattr(self, "ai13_upperband_head_start_hz", 8200.0))
            head_full = float(getattr(self, "ai13_upperband_head_full_hz", 13800.0))
            h_w, n_w, mix_stats = frequency_dependent_mix_weights(
                A_spec, gate_spec, sr, synth_fft, head_start_hz=head_start, head_full_hz=head_full,
            )
            h_raw_w, n_raw_w, raw_mix_stats = frequency_dependent_mix_weights(
            ...
            h_w, n_w, guard_stats = apply_output_rate_mix_guard(h_w, n_w, sr, output_sr)
            h_raw_w, n_raw_w, raw_guard_stats = apply_output_rate_mix_guard(h_raw_w, n_raw_w, sr, output_sr)

            def synth_from_weights(hw, nw):
                sh = spec_h[:, :, :n_spec_frames] * hw[:, :, :n_spec_frames] * S_spec[:, :, :n_spec_frames]
                sn = spec_n[:, :, :n_spec_frames] * nw[:, :, :n_spec_frames] * S_spec[:, :, :n_spec_frames]
                return torch.istft(sh + sn, synth_fft, synth_hop, window=window, length=n_samples).unsqueeze(1)
            ...
            return wav, {
                ...
                "hop_length": synth_hop,
                "harmonic_count": int(harmonic_count),
                "harmonic_ceiling_hz": float(harmonic_ceiling),
                "spectral_bins": int(S_full.shape[1]),
                "upperband_parameter_head_used": True,
                "upperband_parameter_head_revision": 2,
                "upperband_guard_used": True,
                "upperband_head_start_hz": float(head_start),
                "upperband_head_full_hz": float(head_full),
                "upperband_spectral_slope_db_per_oct": float(spectral_head_stats.get("mean_slope_db_per_oct", 0.0)),
                "upperband_spectral_guard_mean": float(spectral_guard_stats.get("mean_upper_spectral_guard", 1.0)),
                "upperband_ap_mean": float(ap_head_stats.get("mean_upper_ap", 0.0)),
                "upperband_ap_raw_mean": float(ap_raw_head_stats.get("mean_upper_ap", 0.0)),
                "upperband_harmonic_weight_mean": float(mix_stats.get("upper_harmonic_weight_mean", 0.0)),
                "upperband_noise_weight_mean": float(mix_stats.get("upper_noise_weight_mean", 0.0)),
                "upperband_mix_scale_mean": float(mix_stats.get("upper_mix_scale_mean", 1.0)),
                "upperband_raw_mix_scale_mean": float(raw_mix_stats.get("upper_mix_scale_mean", 1.0)),
                "upperband_top_harmonic_gain_mean": float(guard_stats.get("top_harmonic_gain_mean", 0.0)),
                "upperband_top_noise_gain_mean": float(guard_stats.get("top_noise_gain_mean", 0.0)),
                "upperband_terminal_start_hz": float(guard_stats.get("noise_terminal_start_hz", 0.0)),
                "upperband_terminal_zero_hz": float(guard_stats.get("terminal_zero_hz", 0.0)),
                "upperband_raw_terminal_zero_hz": float(raw_guard_stats.get("terminal_zero_hz", 0.0)),
            }
        ...
        def forward(self, f0, z, target_samples=None, cond=None, adapter=None, detail=None, prototype_index=None, timbre_shift_semitones=0.0, detail_strength=1.0, frame_controls=None, ai_control_adapter=None, return_aux=False, synthesis_sample_rate=None):
            ...
            if frame_controls:
                # Each learned pack modifies only the axes declared in its metadata.
            ...
            if requested_sr > int(self.sample_rate):
                if bool(getattr(self, "ai13_upperband_guard_enabled", False)):
                    wav, fullband_stats = self._synthesize_fullband_body_upperband_head_v2(
                        f0, S_lin, A_band, A_c_raw, gate, requested_sr
                    )
                elif bool(getattr(self, "ai12_upperband_head_enabled", False)):
...
def blend_dualrate_fullband_body(legacy_output, fullband_output, sr, start_hz=9000.0, full_hz=12100.0):
    ...
    # Match overlap energy conservatively across the dual-rate crossover.
...
def _ai12_bandpass_component(audio, sr, rise_start_hz, rise_full_hz, fall_start_hz, fall_end_hz):
    """Return a smooth band component for band-wise level control."""
...
def blend_dualrate_fullband_body_v2(legacy_output, fullband_output, sr, start_hz=8800.0, full_hz=12100.0):
    """Band-wise 48 kHz body crossover with independent upper-band limits."""
    lo = np.asarray(legacy_output, dtype=np.float32).reshape(-1)
    hi = np.asarray(fullband_output, dtype=np.float32).reshape(-1)
    n = min(lo.size, hi.size)
    if n < 16 or float(sr) * 0.5 <= float(full_hz) + 100.0:
    ...
    # Use progressively lower targets across the seam, presence, and air bands.
    ...
    # Final global safety limit after per-band shaping.
...
def _ai13_tune_band(branch, sr, ref_rms, band, floor_ratio, cap_ratio, max_boost):
    """Tune one crossover band within reference-relative floor and cap limits."""
    component = _ai12_bandpass_component(branch, sr, *band)
    before = float(np.sqrt(np.mean(component.astype(np.float64) ** 2) + 1e-12))
    floor = max(0.0, float(ref_rms) * float(floor_ratio))
    cap = max(floor + 1e-12, float(ref_rms) * float(cap_ratio))
    gain = 1.0
    if before > 1e-10 and floor > 0.0 and before < floor:
        gain = min(float(max_boost), floor / before)
    elif before > cap:
        gain = cap / before
    tuned = np.asarray(branch, dtype=np.float64) + (gain - 1.0) * component.astype(np.float64)
...
def blend_dualrate_fullband_body_v3(legacy_output, fullband_output, sr, start_hz=8200.0, full_hz=13800.0):
    """Wide-overlap full-band crossover with monotonic upper-band targets."""
    lo = np.asarray(legacy_output, dtype=np.float32).reshape(-1)
    hi = np.asarray(fullband_output, dtype=np.float32).reshape(-1)
    n = min(lo.size, hi.size)
    if n < 16 or float(sr) * 0.5 <= float(full_hz) + 100.0:
    ...
    # Reference the body below the analysis-band edge.
    edge = _ai12_bandpass_component(lo, sr, 6200.0, 7000.0, 8800.0, 9600.0)
    edge_rms = max(float(np.sqrt(np.mean(edge.astype(np.float64) ** 2) + 1e-12)), 1e-7)

    bands = [
        ("bridge",   (8600.0,  9500.0, 11200.0, 12100.0), 0.30, 0.90, 1.7),
        ("seam",     (10300.0, 11300.0, 13200.0, 14300.0), 0.20, 0.68, 1.9),
        ("presence", (12600.0, 13800.0, 15800.0, 17100.0), 0.10, 0.46, 2.0),
        ("air",      (15100.0, 16500.0, 18400.0, 19700.0), 0.030, 0.25, 2.0),
        ("top",      (18100.0, 19300.0, 20400.0, 21100.0), 0.0, 0.060, 1.0),
    ]
    stats = {}
    for name, band, floor_ratio, cap_ratio, boost in bands:
        high_branch, gain, before, after = _ai13_tune_band(
            high_branch, sr, edge_rms, band, floor_ratio, cap_ratio, boost,
        )
        stats[f"upperband_{name}_gain"] = float(gain)
        stats[f"upperband_{name}_rms_before"] = float(before)
        stats[f"upperband_{name}_rms_after"] = float(after)

    body_rms = max(float(np.sqrt(np.mean(lo.astype(np.float64) ** 2) + 1e-12)), 1e-7)
    high_rms = float(np.sqrt(np.mean(high_branch.astype(np.float64) ** 2) + 1e-12))
    emergency_limit = body_rms * 0.34
    emergency_gain = 1.0
    if high_rms > emergency_limit:
        emergency_gain = emergency_limit / max(high_rms, 1e-12)
        high_branch *= emergency_gain
    ...
    return out, {
        "used": True,
        "backend": "dual-rate-48k-ddsp-body-v3-slope-continuity-topguard",
        "crossover_start_hz": float(start_hz),
        "crossover_full_hz": float(full_hz),
        "fullband_branch_rms": float(high_rms),
        "fullband_safety_gain": float(emergency_gain),
...
def blend_fullband_highband_refinement(base, foundation_output, continuity_output, sr, strength=1.0):
    """Blend optional high-band refinement onto the full-band synthesis body."""
...
class YuazDDSPResamplerEngine:
    def __init__(self, repo, checkpoint, transition_ms=70.0, use_rvq=False, output_sr=44100, registry_path=None, ddsp_synthesis_sr=48000, fullband_crossover_start_hz=8800.0, fullband_crossover_full_hz=12100.0, ai12_upperband_head_enabled=True, ai12_upperband_head_start_hz=8400.0, ai12_upperband_head_full_hz=12400.0, ai13_upperband_guard_enabled=True, ai13_upperband_head_start_hz=8200.0, ai13_upperband_head_full_hz=13800.0):
        ...
        self.ai13_upperband_guard_enabled = bool(ai13_upperband_guard_enabled)
        self.ai13_upperband_head_start_hz = float(ai13_upperband_head_start_hz)
        self.ai13_upperband_head_full_hz = float(ai13_upperband_head_full_hz)
        ...
        # Select the frequency-dependent 48 kHz parameter path when enabled.
        ...
        # The terminal guard can be disabled independently of the upper-band head.
        self.decoder.ai13_upperband_guard_enabled = bool(self.ai13_upperband_guard_enabled)
        self.decoder.ai13_upperband_head_start_hz = float(self.ai13_upperband_head_start_hz)
        self.decoder.ai13_upperband_head_full_hz = float(self.ai13_upperband_head_full_hz)
        self.decoder.ai13_output_sample_rate = int(self.output_sr)
    ...
    def _models_for_input(self, path):
        # Voicebank-local immutable generations are authoritative; the global registry is a cache.
    ...
    def render(self, req):
        with self.render_lock:
            ...
            if self.ai13_upperband_guard_enabled:
                final, fullband_body_stats = blend_dualrate_fullband_body_v3(
                    legacy_out, fullband_out, self.output_sr,
                    start_hz=self.ai13_upperband_head_start_hz, full_hz=self.ai13_upperband_head_full_hz,
                )
            elif self.ai12_upperband_head_enabled:
            ...
            topband_guard_stats = {"used": False}
            if self.ai13_upperband_guard_enabled and self.output_sr > 40000:
            ...
            return {
                ...
                "ddsp_upperband_guard_enabled": bool(self.ai13_upperband_guard_enabled),
                ...
                "ddsp_upperband_harmonic_ceiling_hz": float(fullband_decode_stats.get("harmonic_ceiling_hz", 0.0)),
                "ddsp_upperband_terminal_start_hz": float(fullband_decode_stats.get("upperband_terminal_start_hz", 0.0)),
                "ddsp_upperband_terminal_zero_hz": float(fullband_decode_stats.get("upperband_terminal_zero_hz", 0.0)),
                "ddsp_topband_guard_used": bool(topband_guard_stats.get("used", False)),
                "ddsp_topband_guard_gain_20k": float(topband_guard_stats.get("terminal_gain_at_20k", 1.0)),
                "ddsp_topband_guard_gain_21k": float(topband_guard_stats.get("terminal_gain_at_21k", 1.0)),
                ...
                "ddsp_upperband_bridge_gain": float(fullband_body_stats.get("upperband_bridge_gain", 1.0)),
                ...
                "ddsp_upperband_top_gain": float(fullband_body_stats.get("upperband_top_gain", 1.0)),
