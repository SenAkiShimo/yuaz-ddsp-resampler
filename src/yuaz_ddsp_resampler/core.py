...
def make_adaptive_decoder_class(base_cls):
    class AdaptiveDDSPDecoder(base_cls):
        ...
        def forward(self, f0, z, target_samples=None, cond=None, adapter=None, detail=None, prototype_index=None):
            if f0.dim() == 2:
                f0 = f0.unsqueeze(1)
            B, _, T = f0.shape
            if adapter is not None:
                z = adapter.apply_latent(z, detail=detail, f0=f0, source_prototype_index=prototype_index)
            ...
            if adapter is not None:
                gate = adapter.apply_gate(gate, f0=f0, source_prototype_index=prototype_index)

            S_lin = self._decompress_envelope(S_c)
            if adapter is not None:
                S_lin = S_lin * adapter.spectral_gain(S_lin.shape[1], device=S_lin.device, dtype=S_lin.dtype, detail=detail, frames=S_lin.shape[-1], f0=f0, batch=S_lin.shape[0], source_prototype_index=prototype_index)
            voiced_frames = (f0 > 1.0).to(f0.dtype)
            A_smooth = self._smooth_ap_bands(A_c_raw)
            if adapter is not None:
                A_smooth = adapter.apply_ap(A_smooth, detail=detail, f0=f0, source_prototype_index=prototype_index)
...
def deterministic_decode(decoder, f0, z, seed, adapter=None, detail=None, prototype_index=None):
    torch.manual_seed(int(seed))
    with torch.inference_mode():
        wav = decoder(f0, z, adapter=adapter, detail=detail, prototype_index=prototype_index)
...
class YuazDDSPResamplerEngine:
    ...
    def render(self, req):
        with self.render_lock:
            ...
            generated = deterministic_decode(self.decoder, f0_t, latent_warp, seed, adapter=adapter, detail=detail_warp, prototype_index=prototype_index)
            ...
            if refiner is not None:
                ...
                generated = refined_t[0, 0].detach().cpu().numpy().astype(np.float32)
                fidelity_residual_rms = float(torch.sqrt(torch.mean(residual_t.pow(2)) + 1e-12).cpu())
