...
class VoicebankPreparer:
    ...
    def __init__(self, project_root, voicebank_root, mode="quick"):
        ...
        self.yuaz_dir = self.voicebank_root / ".yuaz-alpha8-rc3-2"
    ...
    def train(self, valid_caches, manifest_entries):
        ...
        print(
            ...
            f"Using {len(paths)} reconstruction samples and a pool of {len(pairs)} real multipitch pair directions "

            f"({pair_window} pair updates per epoch)."
        ...
        metadata = {
            ...
            "training_stages": {
                ...
                "stage_a_identity": {
                    ...
                    "anti_leak": "selective small-margin prototype routing with mid-band identity preservation",
            ...
            "pair_sampling": "alias_balanced_rotating_wide_first",
    ...
    def train_fidelity_refiner(self, valid_caches):
        ...
        if not bool(self.config.get("enable_fidelity_refiner", False)):
    ...
    def register(self, manifest_entries):
        ...
        refiner_enabled = bool(self.config.get("enable_fidelity_refiner", False))

        loudness_path = self.yuaz_dir / "loudness.json"

        articulation_index_path = self.articulation_dir / "index.json"

        highband_path = self.yuaz_dir / "highband_profiles_v3.json"
        ...
        def add_record(key, item):
            ...
            if not isinstance(record, dict) or record.get("voicebank_id") != self.bank_id:
                ...
                record = {
                    ...
                    "refiner": str(refiner) if (refiner_enabled and refiner.exists()) else "",
                    ...
                    "highband_profiles": str(highband_path) if highband_path.exists() else "",

                    "highband_profile_format": 3 if highband_path.exists() else 0,
                    ...
                    "adapter_generation": "alpha8-rc3-2-two-stage-clarity" if adapter.exists() else "base",
    ...
    def run(self):
        ...
        result = {"profile": profile, "training": metadata, "fidelity": fidelity, "highband": str(highband_path), "highband_stats": highband_db.get("stats", {}), "registry": str(registry), "yuaz_dir": str(self.yuaz_dir)}
