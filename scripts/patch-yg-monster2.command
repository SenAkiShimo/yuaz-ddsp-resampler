...
[ -f "$POST" ] || { echo "Install the monster YG test first." >&2; exit 1; }
cat > "$POST" <<'PY'
import numpy as np
import librosa
...
def _bump(freqs, center, width):
    return np.exp(-0.5 * ((freqs - float(center)) / float(width)) ** 2)
...
def apply_final_gender_formant(audio, sr, target_f0, amount):
    ...
    if amount > 0.0:
        base = min(1.0, 2.0 * a)
        extra = max(0.0, (a - 0.50) / 0.50)
        semitones = 34.0 * base
        warp_gain = 1.30 + 0.55 * base * base + 1.00 * extra * extra
        contour_strength = 0.12 * base + 0.50 * extra
    else:
        base = min(1.0, 10.0 * a)
        extra = max(0.0, (a - 0.10) / 0.90)
        semitones = -(34.0 * base + 14.0 * extra)
        warp_gain = 1.30 + 0.55 * base * base + 1.30 * extra
        contour_strength = 0.14 * base + 0.72 * extra
    ...
    if amount > 0.0:
        contour = (
            0.20 * _bump(freqs, 700.0, 520.0)
            + 0.14 * _bump(freqs, 1350.0, 720.0)
            - 0.17 * _bump(freqs, 2850.0, 1050.0)
            - 0.08 * _bump(freqs, 4700.0, 1450.0)
        )
    else:
        contour = (
            -0.20 * _bump(freqs, 650.0, 500.0)
            - 0.13 * _bump(freqs, 1250.0, 720.0)
            + 0.22 * _bump(freqs, 2650.0, 1050.0)
            + 0.14 * _bump(freqs, 4550.0, 1400.0)
        )
    ...
    delta = warp_gain * (warped - envelope)
    delta = delta + contour_strength * contour[:, None]
    delta = delta * voiced[None, :]
