...
_quiesce_runtime(config,'phonation repin')
generation,staging=begin_generation(bank,'phonation-repin')
try:
    clone_state(source,staging,link_caches=True)
    target=_attach_ai_phonation_foundation(root,staging,source_state=None)
    if target is None:
        meta=json.loads((staging/'ai_phonation_training.ai14.json').read_text(encoding='utf-8'))
        raise RuntimeError(str(meta.get('reason') or 'phonation foundation rejected'))
    _write_base_model_metadata(staging,config)
    final,_=commit_generation(
        bank,generation,staging,
        reason='0.2.8ai.14-phonation-repin',
