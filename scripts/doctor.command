...
print('migration predecessor 0.2.8ai.12:', c.get('previous_028ai12_readonly_state_namespace'))
...
print('ai.12 upper-band fallback:', c.get('ai12_upperband_head_enabled'), c.get('ai12_upperband_head_start_hz'), c.get('ai12_upperband_head_full_hz'))
print('ai.13 slope/top guard:', c.get('ai13_upperband_guard_enabled'), c.get('ai13_upperband_head_start_hz'), c.get('ai13_upperband_head_full_hz'))
assert c.get('engine_version')=='0.2.8ai.13'
assert int(c.get('port'))==47885
assert c.get('runtime_id')=='yuaz-0.2.8ai.13-control-v13'
assert c.get('state_namespace')=='.yuaz-0.2.8ai13'
assert c.get('previous_028ai12_readonly_state_namespace')=='.yuaz-0.2.8ai12'
...
assert c.get('ai13_upperband_guard_enabled') is True
assert float(c.get('ai13_upperband_head_start_hz'))==8200.0
assert float(c.get('ai13_upperband_head_full_hz'))==13800.0
...
WRAP="$HOME/Library/OpenUtau/Resamplers/Yuaz-DDSP-Resampler-v0.2.8ai.13.sh"
...
if [ -d "$HOME/Library/OpenUtau/Resamplers" ]; then
  OLD="$(find "$HOME/Library/OpenUtau/Resamplers" -maxdepth 1 -type f \( -name 'Yuaz-DDSP-Resampler*.sh' -o -name 'Yuaz-DDSP-Resampler*.yaml' \) ! -name 'Yuaz-DDSP-Resampler-v0.2.8ai.13.sh' ! -name 'Yuaz-DDSP-Resampler-v0.2.8ai.13.yaml' -print)"
  if [ -n "$OLD" ]; then
...
echo "PASS: ai.13 slope-continuity/top-band guard enabled; preserved ai.12 upper-band head and ai.11 synthesis/mixer remain available as fallbacks. YH stays a refinement layer."
echo "Doctor OK"
