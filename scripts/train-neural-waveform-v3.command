...
OUTPUT="$ROOT/control_models/neural-waveform-v0.3.0-conditioned-v3.pt"
echo "Using ai.14 manifest: $MANIFEST"
echo "Saving conditioned v3 checkpoints under: $OUTPUT"

exec "$ROOT/.venv/bin/python" -m yuaz_ddsp_resampler.train_neural_waveform_v3 \
  --project-root "$ROOT" \
  --voicebank "$VOICEBANK" \
  --manifest "$MANIFEST" \
  --output "$OUTPUT" \
