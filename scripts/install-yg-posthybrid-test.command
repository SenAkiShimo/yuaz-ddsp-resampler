...
def apply_final_gender_formant(audio, sr, target_f0, amount):
    ...
    smooth_bins = max(9, int(round(430.0 / max(hz_per_bin, 1e-6))))
    if smooth_bins % 2 == 0:
        smooth_bins += 1
    envelope = _smooth_frequency(log_mag, smooth_bins)

    curve = np.sign(amount) * (abs(amount) ** 0.72)
    max_semitones = 10.0
    semitones = max_semitones * curve

    freqs = librosa.fft_frequencies(sr=int(sr), n_fft=n_fft).astype(np.float64)
    low_guard = _smoothstep((freqs - 120.0) / 180.0)
    high_guard = 1.0 - _smoothstep((freqs - 5200.0) / 3800.0)
    ...
    delta = (warped - envelope) * voiced[None, :]
    ratio = np.exp(np.clip(delta, -1.75, 1.75))
    ...
    return out.astype(np.float32), {
        ...
        "voiced_mean": float(np.mean(voiced)) if voiced.size else 0.0,
        "rms_gain": float(gain),
    }
PY
...
echo "Installed post-hybrid YG test"
