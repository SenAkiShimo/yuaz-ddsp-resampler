...
[ -d "$PKG" ] || { echo "Yuaz 0.2.9 runtime not found." >&2; exit 1; }
cat > "$POST" <<'PY'
import numpy as np
import librosa
...
def apply_final_gender_formant(audio, sr, target_f0, amount):
    ...
    curve = np.sign(amount) * (abs(amount) ** 0.85)
    ...
    ratio = np.exp(np.clip(delta, -2.05, 2.05))
    ...
    if rin > 1e-8 and rout > 1e-8:
        out *= float(np.clip(rin / rout, 0.68, 1.46))

    return out.astype(np.float32), {"used": True, "amount": float(amount), "semitones": float(semitones)}
PY
...
echo "Restored extreme YG runtime"
