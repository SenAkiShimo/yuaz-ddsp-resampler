...
echo "v0.4 robust voicebank training"
echo "Voicebank: $VOICEBANK"
echo "Manifest:  $MANIFEST"
echo "Recorded cross-pitch pairs will be used where available."
echo "Missing pitch buckets will be filled with synthetic high-quality pitch-shift pairs."
echo "Checkpoints are isolated per voicebank; stale checkpoints from another bank are never reused."

exec "$ROOT/.venv/bin/python" -m yuaz_ddsp_resampler.train_neural_waveform_v04 \
  --project-root "$ROOT" \
  --voicebank "$VOICEBANK" \
  --manifest "$MANIFEST" \
  "${@:2}"
