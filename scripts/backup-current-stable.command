...
DEST="$BASE/$STAMP-before-0.2.8ai.13"
mkdir -p "$DEST"
...

{
  ...
  echo "policy=pre-purge snapshots; 0.2.8ai.12, 0.2.8ai.11, 0.2.8ai.10, 0.2.8ai.9, 0.2.8ai.8, 0.2.8ai.7, 0.2.8ai.6, 0.2.8ai.5, 0.2.8ai.4, 0.2.8ai.3, 0.2.8ai.2, 0.2.8ai.1, 0.2.8ai, 0.2.7 AI.3 and RC4.2 are not stopped, removed, overwritten, or modified"
} > "$DEST/manifest.txt"
...
snapshot_runtime \
  "0.2.8ai.12" \
  "$APP/0.2.8ai.12" \
  "$OU/Yuaz-DDSP-Resampler-v0.2.8ai.12.sh" \
  "$OU/Yuaz-DDSP-Resampler-v0.2.8ai.12.yaml"
...
# Snapshot every current ai.12 voicebank state before the installer is allowed to purge it.
...
seen=set()
for state in root.rglob('.yuaz-0.2.8ai12'):
    if not state.is_dir():
        continue
    ...
    archive=out/f'{safe}-0.2.8ai12-state.tar.gz'
    def filt(info):
        rel=Path(info.name)
    ...
    with tarfile.open(archive,'w:gz') as tf:
        tf.add(state, arcname='.yuaz-0.2.8ai12', filter=filt)
    with (out/'manifest.txt').open('a',encoding='utf-8') as f:
        f.write(f'{bank}\t{archive.name}\n')
print(f'Voicebank ai.12 safety snapshots: {len(seen)}')
PY_STATE
fi

for CAND in \
  "$HOME/Downloads/Yuaz_DDSP_v0.2.8ai.12.zip" \
  "$HOME/Downloads/yuaz-ddsp-resampler-v0.2.8ai.12.zip" \
...
echo "$DEST" > "$BASE/LATEST_PRE_0.2.8AI13_ENGINE_SNAPSHOT.txt"
echo "Pre-0.2.8ai.13 engine snapshot created: $DEST"
echo "Snapshot includes current 0.2.8ai.12 and available predecessors before migration/purge. Previous installed versions may be removed after successful state migration. Included when present: 0.2.8ai.12 + 0.2.8ai.11 + 0.2.8ai.10 + 0.2.8ai.9 + 0.2.8ai.8 + 0.2.8ai.7 + 0.2.8ai.6 + 0.2.8ai.5 + 0.2.8ai.4 + 0.2.8ai.3 + 0.2.8ai.2 + 0.2.8ai.1 + 0.2.8ai + 0.2.7 AI.3 + RC4.2 runtimes/wrappers + existing voicebank state."
