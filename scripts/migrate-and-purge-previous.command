...
REG="$APP/state/voicebank_registry-0.2.8ai13.json"

echo "Yuaz 0.2.8ai.13 migration + previous-version purge"
echo "First migrates the newest valid prepared state into .yuaz-0.2.8ai13."
echo "0.2.8ai.12 is the preferred migration source; adapter/Fidelity/articulation/high-band weights are preserved."
...
from yuaz_ddsp_resampler.state import (
    STATE_CONTAINER, PREVIOUS_028AI12_STATE_CONTAINER, PREVIOUS_028AI11_STATE_CONTAINER, PREVIOUS_028AI10_STATE_CONTAINER, PREVIOUS_028AI9_STATE_CONTAINER,
...
legacy_names=(
    PREVIOUS_028AI12_STATE_CONTAINER, PREVIOUS_028AI11_STATE_CONTAINER, PREVIOUS_028AI10_STATE_CONTAINER, PREVIOUS_028AI9_STATE_CONTAINER, PREVIOUS_028AI8_STATE_CONTAINER,
...
for D in \
  "$APP/0.2.8ai.12" "$APP/0.2.8ai.11" "$APP/0.2.8ai.10" "$APP/0.2.8ai.9" "$APP/0.2.8ai.8" "$APP/0.2.8ai.7" "$APP/0.2.8ai.6" "$APP/0.2.8ai.5" "$APP/0.2.8ai.4" "$APP/0.2.8ai.3" \
...
if [ -d "$DEST" ]; then
  find "$DEST" -maxdepth 1 -type f \
    ...
    ! -name 'Yuaz-DDSP-Resampler-v0.2.8ai.13.yaml' -delete
fi

rm -f "$APP/state"/voicebank_registry-0.2.8ai12.json \
      "$APP/state"/voicebank_registry-0.2.8ai11.json \
      "$APP/state"/voicebank_registry-0.2.8ai10.json \
...
if [ -d "$SINGERS" ]; then
  find "$SINGERS" -type d \( \
    -name '.yuaz-0.2.8ai12' -o -name '.yuaz-0.2.8ai11' -o -name '.yuaz-0.2.8ai10' -o -name '.yuaz-0.2.8ai9' -o -name '.yuaz-0.2.8ai8' -o -name '.yuaz-0.2.8ai7' -o -name '.yuaz-0.2.8ai6' -o -name '.yuaz-0.2.8ai5' -o -name '.yuaz-0.2.8ai4' -o \
...
echo "Preserved: .yuaz-0.2.8ai13, source WAV/OTO, datasets, and ~/Documents/Yuaz-DDSP-Backups."
