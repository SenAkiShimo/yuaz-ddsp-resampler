...
WARM="$ROOT/control_models/neural-waveform-v0.3.0-conditioned-v3-pareto-best.pt"
[ -f "$WARM" ] || { echo "Missing v3 pareto warm-start checkpoint: $WARM"; exit 1; }
export YUAZ_V4_WARM_START="$WARM"

OUTPUT="$ROOT/control_models/neural-waveform-v0.3.0-conditioned-v4.pt"
echo "Using ai.14 manifest: $MANIFEST"
echo "Warm-starting v4 from: $WARM"
echo "Saving conditioned v4 checkpoints under: $OUTPUT"
echo "WORLDLINE-R reference is diagnostic only; it is not used as a training target."

exec "$ROOT/.venv/bin/python" -m yuaz_ddsp_resampler.train_neural_waveform_v4 \
  --project-root "$ROOT" \
  --voicebank "$VOICEBANK" \
  --manifest "$MANIFEST" \
  --output "$OUTPUT" \
